...
(def fixtures [{:room/title "World domination"     :room/source "web/rooms/world_domination.edn"}
...
(defn- ajax [url callback]
  (.send goog.net.XhrIo url
         (fn [reply]
           (-> (j/call-in reply [:target :getResponseText])
...
(defmethod base/do! :mock-api/request
