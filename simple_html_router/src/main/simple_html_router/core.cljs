...
(defmethod base/view ::home-page
  ...
  [:span.main
   ...
   [:ul
    [:li [:a {:href (router :path-for ::items-page)} "Items of hello-router"]]
  ...
  [:span.main
   ...
   [:ul (map (fn [item-id]
               [:li {:name (str "item-" item-id) :key (str "item-" item-id)}
                [:a {:href (router :path-for ::item-page {:item-id item-id})} "Item: " item-id]])
             (range 1 60))]])
...
(defmethod base/view ::item-page
  ...
  (let [item-id (get-in path-params [:item-id])]
    ...
    [:span.main
     ...
     [:p [:a {:href (router :path-for ::items-page)} "Back to the list of items"]]]))
...
(defmethod base/view ::current-page
  ...
  (let [current-route-value @(router :current-route-atom)
        page-name (:page-name current-route-value)]
    [:div
     [:header
      [:p [:a {:href (router :path-for ::home-page)} "Home"] " | "
       [:a {:href (router :path-for ::about-page)} "About hello-router"]]]
